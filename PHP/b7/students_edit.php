...
if ($id){
    $data = get_student($id); // You will need to modify get_student() in students.php to use PDO
}
...
if (!empty($_POST['edit_student']))
{
    ...
    if (!$errors){
        edit_student($data['sv_id'], $data['sv_name'], $data['sv_sex'], $data['sv_birthday']); // Make sure edit_student uses PDO
